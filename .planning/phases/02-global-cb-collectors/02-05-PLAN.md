...
<objective>
Create People's Bank of China collector with ROBUST FRED fallback.

Purpose: PBoC has no public API. Primary approach scrapes official HTM/XLS files. **ROBUST FALLBACK**: Uses FRED TRESEGCNM052N (China foreign reserves) as a proxy, matching Apps Script v3.4.1 approach.

**CONFIDENCE: HIGH** - The FRED fallback is the same approach that Apps Script v3.4.1 uses successfully. Foreign reserves are a reasonable proxy for CB liquidity impact.

Robust strategy (see 02-ROBUST-STRATEGIES.md):
- Tier 1: Scrape PBoC website (may fail - Chinese gov sites change)
- Tier 2: FRED TRESEGCNM052N (**RELIABLE** - same as Apps Script)
- Tier 3: Cached baseline (GUARANTEED)

Output: PBOCCollector class that ALWAYS returns data via fallback chain.
</objective>
...
class PBOCCollector(BaseCollector[pd.DataFrame]):
    ...
    BASELINE_VALUE = 47_296_970  # 100 million CNY units (~47.3 trillion CNY, Nov 2025)
...
- PBOCCollector class with ROBUST multi-tier fallback
