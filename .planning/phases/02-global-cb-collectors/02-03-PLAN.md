...
<objective>
Create Bank of England collector with ROBUST multi-tier fallback.

Purpose: FRED series BOEBSTAUKA was discontinued in 2016. BoE database API returns 403 for programmatic access. This collector implements a multi-tier fallback strategy to ensure data is ALWAYS returned.
...
- Total assets ~£848 billion (Nov 2025), calculated by summing components
- XML API exists but current series codes unknown (old ones discontinued)

**ROBUST STRATEGY** (see 02-ROBUST-STRATEGIES.md):
- Tier 1: Scrape weekly report HTML (may fail)
- Tier 2: Try BoE XML API if series codes found
- Tier 3: Use FRED UK M4 as correlation proxy
- Tier 4: Return cached baseline with stale flag (GUARANTEED)

Output: BOECollector class with multi-tier fallback, ALWAYS returns data.
</objective>
...
class BOECollector(BaseCollector[pd.DataFrame]):
    ...
    async def collect(
        ...
        try:
            ...
            logger.warning(f"BoE Tier 1 (scraping) failed: {e}")

        # Tier 2: Try FRED UK M4 proxy (correlation-based estimate)
        try:
            return await self._collect_via_fred_proxy(start_date, end_date)
        except Exception as e:
            logger.warning(f"BoE Tier 2 (FRED proxy) failed: {e}")
    ...
    def _get_cached_baseline(self) -> pd.DataFrame:
        ...
        return pd.DataFrame({
            ...
            "unit": ["millions_gbp"],
            "stale": [True],  # Flag indicating this is cached data
        })
...
- Multi-tier fallback implemented (scraping → FRED proxy → cached baseline)
- ALWAYS returns data (never fails completely)
- Each fallback tier logs warnings for monitoring
- Data includes "source" field indicating which tier succeeded
- Data includes "stale" flag when using cached baseline
- Integration tests verify all fallback tiers
</success_criteria>
