...
SERIES_MAP: dict[str, str] = {
    "boc_total_assets": "TBD",  # Discover via ValetInterpreter().get_all_series()
}

UNIT_MAP: dict[str, str] = {
    # Will be populated after discovering series
}
...
class BOCCollector(BaseCollector[pd.DataFrame]):
    ...
    def discover_series(self) -> pd.DataFrame:
        ...
        return self._client.get_all_series()
...
<task type="auto">
  ...
  <action>
1. Use BOCCollector.discover_series() to find available series
2. Identify the series ID for BoC total assets (look for "balance sheet", "total assets", or similar)
3. Update SERIES_MAP with the correct series ID
4. Implement _fetch_sync() to fetch and normalize the data
5. Add integration test for BOCCollector

Expected BoC series patterns (from research):
- Look for series related to BoC balance sheet assets
- Data likely in millions CAD
- Weekly or monthly frequency

Add test that verifies:
- BOCCollector can discover available series
- BOCCollector can fetch balance sheet data
- Data is normalized to standard format (timestamp, series_id, source, value, unit)
  </action>
  <verify>uv run pytest tests/integration/test_global_cb_collectors.py::test_boc_collector -v --tb=short</verify>
  <done>BoC series identified, implemented, and tested</done>
</task>
